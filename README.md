...
Установка резпзитоиря и сознадине и окружения.
...
```shell
python -m vlm_guidance_project_multi_pipeline_optim.vlm_guidance.batch_run batch.prompts_file=datasets/subset.txt run.vqa_score=True run.vanilla_sd=True run.flux1=True batch.output_root_dir=subset_generations    
```
...
```shell
python3 metrics/clip_score_clalc.py -generations subset_generations --output metrics/clip_score_subset_result.csv
```
...
python3 metrics/alignment_score_clalc.py \
  -generations subset_generations \
